...
ScratchpadDatapath::ScratchpadDatapath(std::string bench,
  ...
  scratchpad = new Scratchpad(
      user_params.scratchpad_read_ports, user_params.scratchpad_write_ports,
      user_params.cycle_time, user_params.ready_mode, user_params.policy);
...
void ScratchpadDatapath::clearDatapath() { BaseDatapath::clearDatapath(); }
...
void ScratchpadDatapath::completePartition() {
  ...
  for (auto it = user_params.partition.begin();
       it != user_params.partition.end(); ++it) {
...
void ScratchpadDatapath::scratchpadPartition() {
  ...
  for (auto part_it = user_params.partition.begin();
       part_it != user_params.partition.end(); ++part_it) {
    ...
    if (!scratchpad_partition_executed) {
      ...
      scratchpad->setScratchpad(array_label, base_addr, part_type, p_factor,
                                size, wordsize);
      std::cout << "Setting the ScratchPad" << std::endl;
      std::cout
          << " array_label, base_addr, part_type, p_factor, size, wordsize"
          << std::endl;
      std::cout << array_label << "," << base_addr << "," << part_type << ","
                << p_factor << "," << size << "," << wordsize << std::endl;
    } else {
  ...
  while (it != executingQueue.end()) {
    ...
    if (node->is_memory_op()) {
      ...
      if (registers.has(array_name)) {
        ...
        try {
          if (scratchpad->canServicePartition(array_name, array_name_index,
                                              vaddr, isLoad)) {
            markNodeStarted(node);
            if (isLoad) {
...
void ScratchpadDatapath::updateChildren(ExecNode *node) {
  ...
  for (boost::tie(out_edge_it, out_edge_end) =
           out_edges(curr_vertex, program.graph);
...
int ScratchpadDatapath::writeConfiguration(sql::Connection *con) {
  ...
  getCommonConfigParameters(unrolling_factor, pipelining_factor,
                            partition_factor);
...
void ScratchpadDatapath::writeOtherStats() {
  ...
  spad_stats_file.open(file_name.c_str(),
