...
BaseDatapath::BaseDatapath(std::string &bench, std::string &trace_file_name,
...
void BaseDatapath::updateUnrollingPipeliningWithLabelInfo(
  ...
  for (auto it = program.labelmap.begin(); it != program.labelmap.end(); ++it) {
    ...
    UniqueLabel label_without_num(label_with_num.get_function(),
                                  label_with_num.get_label(), 0);
...
void BaseDatapath::writePerCycleActivity() {
  ...
  initPerCycleActivity(comp_partition_names, mem_partition_names, mem_activity,
                       func_activity, func_max_activity, num_cycles);
  ...
  outputPerCycleActivity(comp_partition_names, mem_partition_names,
                         mem_activity, func_activity, func_max_activity);
}
...
void BaseDatapath::initPerCycleActivity(
  ...
  for (auto it = comp_partition_names.begin(); it != comp_partition_names.end();
       ++it) {
    mem_activity.insert({*it, std::vector<MemoryActivity>(num_cycles, {0, 0})});
  }
...
void BaseDatapath::updatePerCycleActivity(
  ...
  for (auto node_it = program.nodes.begin(); node_it != program.nodes.end();
    ...
    FunctionActivity &curr_fu_activity =
        func_activity.at(func_id).at(node_level);

    if (node->is_multicycle_op()) {
      for (int stage = 0;
           node_level + stage < node->get_complete_execution_cycle(); stage++) {
  ...
  for (auto it = functionNames.begin(); it != functionNames.end(); ++it) {
    ...
    max_it->second.mul =
        std::max_element(
            cycle_activity.begin(), cycle_activity.end(),
...
void BaseDatapath::outputPerCycleActivity(
  ...
  getSinglePrecisionFloatingPointMultiplierPowerArea(
      cycleTime, &fp_sp_mul_int_power, &fp_sp_mul_switch_power,
      &fp_sp_mul_leak_power, &fp_sp_mul_area);
  getDoublePrecisionFloatingPointMultiplierPowerArea(
      cycleTime, &fp_dp_mul_int_power, &fp_dp_mul_switch_power,
      &fp_dp_mul_leak_power, &fp_dp_mul_area);
  getSinglePrecisionFloatingPointAdderPowerArea(
      cycleTime, &fp_sp_add_int_power, &fp_sp_add_switch_power,
      &fp_sp_add_leak_power, &fp_sp_add_area);
  getDoublePrecisionFloatingPointAdderPowerArea(
      cycleTime, &fp_dp_add_int_power, &fp_dp_add_switch_power,
      &fp_dp_add_leak_power, &fp_dp_add_area);
  getTrigonometricFunctionPowerArea(cycleTime, &trig_int_power,
                                    &trig_switch_power, &trig_leak_power,
                                    &trig_area);
  ...
  for (unsigned curr_level = 0; ((int)curr_level) < num_cycles; ++curr_level) {
    ...
    for (auto it = functionNames.begin(); it != functionNames.end(); ++it) {
...
#ifdef DEBUG
      stats << curr_activity.fp_sp_mul << "," << curr_activity.fp_dp_mul << ","
            << curr_activity.fp_sp_add << "," << curr_activity.fp_dp_add << ","
            << curr_activity.mul << "," << curr_activity.add << ","
            << curr_activity.bit << "," << curr_activity.shifter << ","
            << curr_activity.trig << ",";
#endif
      ...
      float curr_trig_dynamic_power =
          (trig_switch_power + trig_int_power) * curr_activity.trig;
      float curr_mul_dynamic_power =
          (mul_switch_power + mul_int_power) * curr_activity.mul;
      float curr_add_dynamic_power =
          (add_switch_power + add_int_power) * curr_activity.add;
      float curr_bit_dynamic_power =
          (bit_switch_power + bit_int_power) * curr_activity.bit;
      float curr_shifter_dynamic_power =
          (shifter_switch_power + shifter_int_power) * curr_activity.shifter;
      fu_dynamic_energy +=
    ...
    for (auto it = comp_partition_names.begin();
      ...
      int reg_words_written =
          ceil(((float)reg_bytes_written) / reg->getWordSize());
    ...
    for (auto it = mem_partition_names.begin(); it != mem_partition_names.end();
      ...
      int mem_bytes_read =
          mem_activity.at(array_name).at(curr_level).bytes_read;
      int mem_bytes_written =
          mem_activity.at(array_name).at(curr_level).bytes_write;
...
void BaseDatapath::writeSummary(std::ostream &outfile,
  ...
  outfile << "Top level function: " << summary.topLevelFunctionName
          << std::endl;
  outfile << "Cycle : " << summary.num_cycles << " cycles" << std::endl;
  outfile << "Avg Power: " << summary.avg_power << " mW" << std::endl;
  outfile << "Idle FU Cycles: " << summary.idle_fu_cycles << " cycles"
          << std::endl;
...
void BaseDatapath::writeBaseAddress() {
  ...
  for (auto it = program.nodes.begin(), E = program.nodes.end(); it != E;
       ++it) {
    ...
    if (partitioned_label.empty())
      continue;
    int num_fields = sscanf(partitioned_label.c_str(), "%[^-]-%d",
                            original_label, &partition_id);
    if (num_fields != 2)
      continue;
    gzprintf(gzip_file, "node:%u,part:%s,base:%lld\n", it->first,
             partitioned_label.c_str(), getBaseAddress(original_label));
  }
  gzclose(gzip_file);
}
...
void BaseDatapath::dumpGraph(std::string graph_name) {
  ...
  BGL_FORALL_VERTICES(v, program.graph, Graph) {
    ExecNode *node =
        program.nodes.at(get(boost::vertex_node_id, program.graph, v));
...
int BaseDatapath::rescheduleNodesWhenNeeded() {
  ...
  for (auto vi = topo_nodes.begin(); vi != topo_nodes.end(); ++vi) {
    ...
    for (boost::tie(in_i, in_end) = in_edges(*vi, program.graph);
         in_i != in_end; ++in_i) {
  ...
  while (!config_file.eof()) {
    ...
    if (!type.compare("flatten")) {
      ...
      if (wholeline.find("complete") == std::string::npos) {
        sscanf(rest_line.c_str(), "%[^,],%[^,],%d,%d,%d\n", part_type,
               array_label, &size, &wordsize, &p_factor);
        m_type = spad;
        ...
        sscanf(rest_line.c_str(), "%[^,],%[^,],%d\n", part_type, array_label,
               &size);
        p_type = complete;
        m_type = reg;
      }
      user_params.partition[array_label] = {m_type,   p_type,   size,
                                            wordsize, p_factor, 0};
      ...
      user_params.partition[array_label] = {cache,    none,     size,
                                            wordsize, p_factor, 0};
