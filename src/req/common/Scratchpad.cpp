...
Scratchpad::Scratchpad(
    unsigned _read_ports, unsigned _write_ports, float cycle_time, bool _ready_mode, std::string& _policy) {
  read_ports = _read_ports;
  write_ports= _write_ports;
...
void Scratchpad::setScratchpad(std::string baseName,
  ...
  LogicalArray* curr_base = new LogicalArray(
      ...
      read_ports,write_ports, ready_mode,policy);
...
void Scratchpad::getAveragePower(unsigned int cycles,
  ...
  for (auto it = logical_arrays.begin(); it != logical_arrays.end(); ++it) {
    std::cout<<"part index is"<<it->second->getNumPartitions();;
    std::cout<<"load energy is "<<it->second->getReadEnergy();
    load_energy += it->second->getNumPartitions()*it->second->getReadEnergy();
    store_energy +=  it->second->getNumPartitions()*it->second->getWriteEnergy();
    leakage_power +=  it->second->getNumPartitions()*it->second->getLeakagePower();
  }
