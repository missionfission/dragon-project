...
LogicalArray::LogicalArray(std::string _base_name,
                           ...
                           unsigned _word_size,
                           unsigned _read_ports,
                           unsigned _write_ports,
                           bool _ready_mode, std::string& _policy)
    : base_name(_base_name), base_addr(_base_addr),
      partition_type(_partition_type), num_partitions(_partition_factor),
      total_size(_total_size), word_size(_word_size), read_ports(_read_ports),write_ports(_write_ports), policy(_policy) {
  ...
  if((read_ports>1 || write_ports>1) && part_size>=1024)
    {
    std::cout<<"read ports"<<read_ports<<" write_ports"<<write_ports<<" "<<part_size<<std::endl;
    uca_org_t cacti_result= multiWrapper(part_size,word_size, read_ports,  write_ports, policy);
    ...
    {uca_org_t cacti_result = cactiWrapper(part_size, word_size, read_ports);
    part_read_energy = cacti_result.power.readOp.dynamic * 1e+12;
    part_write_energy = cacti_result.power.writeOp.dynamic * 1e+12;
    part_leak_power = cacti_result.power.readOp.leakage * 1000;
    part_area = cacti_result.area;
  ...
  for (auto part_size : size_per_part) {
    if (_ready_mode) {
      ...
      curr_part->setNumPorts(read_ports, write_ports);
