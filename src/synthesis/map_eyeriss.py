...
def gen_nested_loop_desc(layer, batch_size, dim_array):
    ...
    def repl_fold(layer, dim_lpeset, dim_array):
        ...
        dim_ppeset_frac = PhyDim2(
            Util.idivc(dim_lpeset.h, fold_h), Util.idivc(dim_lpeset.w, fold_w)
        )
        # Physical PE set dimensions.
        dim_ppeset = PhyDim2(
            dim_ppeset_frac.h * repl_h * f_w2h,
            Util.idivc(dim_ppeset_frac.w * repl_w, f_w2h),
        )

        return dim_ppeset, repl, fold, dim_ppeset_frac

    dim_ppeset, repl, fold, dim_ppeset_frac = repl_fold(layer, dim_lpeset, dim_array)

    if not (dim_ppeset.h <= dim_array.h and dim_ppeset.w <= dim_array.w):
        raise RuntimeError(
            "MapEyeriss: dim_ppeset with size {} does not fit "
            "in dim_array with size {}".format(dim_ppeset, dim_array)
        )
    ...
    if resource_utilization < 0.25:
        raise RuntimeError(
            "MapEyeriss: array resource utilization is < 25%. "
    ...
    num_rows_ppeset_frac[de.IFM] = (
        num_rows_ppeset_frac[de.FIL] + (num_rows_ppeset_frac[de.OFM] - 1) * layer.strd
    )
    ...
    num_rows_ppeset_frac[de.IFM] = min(
        num_rows_ppeset_frac[de.IFM], dim_ppeset_frac.size()
    )
    # print repl
    for t_repl_h in Util.factorize(repl.h, 2):
        ...
        ## Loop trip counts.
        ## Yunfeng: uncommented 4 lines below for debugging
        ...
        # the same.
        usize_gbuf[de.FIL] = (
            num_rows_ppeset_frac[de.FIL]
            * layer.sfil
            * (cnt_ifms_per_ppeset * cnt_ofms_per_ppeset * cnt_ppesets_per_procpass)
        )
        usize_gbuf[de.IFM] = (
            num_rows_ppeset_frac[de.IFM] * layer.sifm * cnt_ifms_per_ppeset
        )
        usize_gbuf[de.OFM] = (
            num_rows_ppeset_frac[de.OFM] * layer.sofm * cnt_ofms_per_ppeset
        )
        ...
        if not all([x >= 0 for x in unit_access[me.ITCN]]):
            raise RuntimeError(
                "MapEyeriss: encounter negative access count to "
        ...
        ## Yunfeng: uncommented the line below for debugging
        ...
        yield LoopBlocking.NestedLoopDesc(
            lcnt_ifm,
            lcnt_ofm,
            lcnt_bat,
            usize_gbuf,
