...
# import synthesisDataCategoryEnum as de
# import LoopBlocking
# import Util
...
def _solve_lpbl_iofmap_gbuf_reside(resource, nested_loop_desc, reside_dce):
    ...
    def goal(ty, tb):  # pylint: disable=invalid-name
        ...
        return (
            (nfmaps_x * usize_gbuf_x * nbats) * facc_x * ty
    ...
    def constraints(ty, tb):  # pylint: disable=invalid-name
        ...
        c1 = (nfmaps_y * usize_gbuf_y / float(ty)) * (nbats / float(tb)) < max_size_gbuf
        return c1
    ...
    ty_top = nfmaps_y * math.sqrt(
        float(usize_gbuf_fil * usize_gbuf_y) / (usize_gbuf_x * max_size_gbuf * facc_x)
    )
    tb_top = nbats * math.sqrt(
        float(usize_gbuf_x * usize_gbuf_y * facc_x) / (usize_gbuf_fil * max_size_gbuf)
    )
    ...
    best_ty_tb = min(
        [(goal(*ty_tb_),) + ty_tb_ for ty_tb_ in ty_tb_cands if constraints(*ty_tb_)]
    )
...
def _solve_lpbl_filter_gbuf_reside(resource, nested_loop_desc):
    ...
    for dce_y in [de.IFM, de.OFM]:
        ...
        ny_top_cand = min(
            float(max_size_regf - usize_regf_x) / (usize_regf_y + usize_regf_fil),
        ...
        # Goal value.
        goal_val = (
            (nfmaps_x * usize_gbuf_x * nbats) * facc_x * ty_top
            + (nfmaps_y * usize_gbuf_y * nbats) * facc_y
        ...
        c1 = (
            1 * usize_regf_x * 1
            + ny_top * usize_regf_y * 1
            + 1 * ny_top * usize_regf_fil
            <= max_size_regf
        )
...
def gen_loopblocking_gbuf_regf(
    ...
    for reside_dce in range(de.NUM):
        if all(
            [
                denum == reside_dce or options.allow_gbuf_bypass[denum]
