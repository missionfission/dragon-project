...
class Partition2dScheme(object):
    ...
    def gen_all_indexes2d(self):
        ...
        for indexes in itertools.product(
            *[itertools.product(*[range(dim) for dim in p]) for p in self.partition2d]
        ):
    ...
    def physical_coordinate2d(self, index2d):
        ...
        for penum in self.order:
            coord = [
                c * p + i
                for c, p, i in zip(coord, self.partition2d[penum], index2d[penum])
            ]
...
def gen_layer_partition2d(layer, dim_nodes):
    ...
    parameters.
    """
    for ph, pw in itertools.product(
        Util.factorize(dim_nodes.h, pe.NUM), Util.factorize(dim_nodes.w, pe.NUM)
    ):
        ...
        if not Util.approx_dividable(layer.nofm, partition2d[pe.OUTP].size()):
            continue
        if not Util.approx_dividable(
            layer.sofm, partition2d[pe.OFMP].h
        ...
        for order in itertools.permutations(tuple(range(pe.NUM))):
            ...
            no_partitions = [v for v in range(pe.NUM) if partition2d[v].size() == 1]
            if not all(
                [order[i] == no_partitions[i] for i in range(len(no_partitions))]
            ):
                continue
...
def unit_nhops_layer_partition2d(layer, batch_size, part_lcurr, part_lprev):
    ...
    req_lists_lprev = [
        [[[] for _ in range(layer.sifm)] for _ in range(layer.sifm)]
